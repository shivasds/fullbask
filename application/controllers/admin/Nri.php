...
class Nri extends Admin_Controller {
    ...
    function __construct()
    {
        ...
        $this->load->model('Nri_model');

        // set constants
        define('REFERRER', "referrer");
        define('THIS_URL', base_url('admin/nri'));
                   ...
                   if ($k === 'closer_look_images'){
                        if ($this->input->post('closer_look_images')){
                            ...
                            if (isset($closerImagesWithCaption['caption'])){
                                $currentItmes = $this->Nri_model->getWhere(array('name' => 'closer_look_images'),
                                    'about_us_images');
//                                debug_continue($currentItmes);debug($closerImagesWithCaption);
                                foreach ($closerImagesWithCaption['caption'] as $index => $item) {
                                    if (isset($currentItmes[$index])){
                                        ...
                                        $this->Nri_model->insertRow(array('name' => $k, 'image' => (isset($closerImagesWithCaption['images'][$index]) ? $closerImagesWithCaption['images'][$index] : ''), 'title' =>  $item), 'about_us_images');
                                    }
                                }
                            }
                        }
                       ...
                       foreach ($v as $image) {
                           $this->Nri_model->insertRow(array('name' => $k, 'image' => $image), 'about_us_images');
                       }
                   }
                }else{/** We have a key - value pair */
                    /** Insert option only if the value is available */
                    if ($this->Nri_model->getOneWhere(array('name' => $k)) == null) {
                        $this->Nri_model->insertRow(array('name' => $k, 'value' => $v));
                    }else{
                        $this->Nri_model->updateWhere(array('name' => $k),array( 'value' => $v));
                    }
                }
            }
            $this->session->set_flashdata('success', 'About Us Updated Successfully');
            redirect(site_url('admin/Nri'));
        }
        ...
        $data['content'] = $this->load->view('admin/Nri', $content_data, TRUE);
        $this->load->view($this->template, $data);
    }
    public function get_images($table_of_image = 'about_us_images') {
        $result = array();
        $images = $this->Nri_model->getWhere(array('name'=>$this->input->get('type')),$table_of_image);
    ...
    public function delete_files($table_of_image = 'about_us_images') {
        ...
        $this->Nri_model->deleteWhere(array('image' => $path), $table_of_image);
        echo unlink('./' . $path);
    }
