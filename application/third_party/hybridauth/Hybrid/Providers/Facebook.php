...
class Hybrid_Providers_Facebook extends Hybrid_Provider_Model
{
	...
	*/
	function getUserProfile()
	{
		...
		$this->user->profile->coverInfoURL  = "http://graph.facebook.com/" . $this->user->profile->identifier . "?fields=cover&access_token=".$this->api->getAccessToken();
	...
	*/
	function getUserContacts()
	{
		...
		foreach( $returnedContacts as $item ){
			...
			$uc->photoURL    = "http://graph.facebook.com/" . $uc->identifier . "/picture?width=150&height=150";
	...
	function getUserActivity( $stream )
	{
		...
		foreach( $response['data'] as $item ){
			...
			if( ! empty( $ua->text ) ){
				...
				$ua->user->photoURL     = "http://graph.facebook.com/" . $ua->user->identifier . "/picture?type=square";
