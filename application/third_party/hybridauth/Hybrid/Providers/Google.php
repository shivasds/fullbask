...
class Hybrid_Providers_Google extends Hybrid_Provider_Model_OAuth2
{
	...
	// default permissions 
	public $scope = "http://www.googleapis.com/auth/plus.login http://www.googleapis.com/auth/plus.profile.emails.read http://www.google.com/m8/feeds/";
	...
	*/
	function initialize() 
	{
		...
		$this->api->authorize_url  = "http://accounts.google.com/o/oauth2/auth";
		$this->api->token_url      = "http://accounts.google.com/o/oauth2/token";
	...
	*/
	function getUserProfile()
	{
		...
		if (strpos($this->scope, '/auth/plus.profile.emails.read') !== false) {
			$verified = $this->api->api( "http://www.googleapis.com/plus/v1/people/me" );

			if ( ! isset( $verified->id ) || isset( $verified->error ) )
				$verified = new stdClass();
		} else {
			$verified = $this->api->api( "http://www.googleapis.com/plus/v1/people/me/openIdConnect" );

			if ( ! isset( $verified->sub ) || isset( $verified->error ) )
				$verified = new stdClass();
		}

		$response = $this->api->api( "http://www.googleapis.com/plus/v1/people/me" );
	...
	*/
	...
	{ 
		...
		if (strpos($this->scope, '/auth/plus.login') !== false) {
			...
			$response = $this->api->api( "http://www.googleapis.com/plus/v1/people/me/people/visible?" 
