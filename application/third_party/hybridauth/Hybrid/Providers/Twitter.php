...
class Hybrid_Providers_Twitter extends Hybrid_Provider_Model_OAuth1
{
	...
	*/
	function initialize()
	{
		...
		$this->api->api_base_url      = "http://api.twitter.com/1.1/";
		$this->api->authorize_url     = "http://api.twitter.com/oauth/authenticate";
		$this->api->request_token_url = "http://api.twitter.com/oauth/request_token";
		$this->api->access_token_url  = "http://api.twitter.com/oauth/access_token";

		if ( isset( $this->config['api_version'] ) && $this->config['api_version'] ){
			$this->api->api_base_url  = "http://api.twitter.com/{$this->config['api_version']}/";
		}
 
		if ( isset( $this->config['authorize'] ) && $this->config['authorize'] ){
			$this->api->authorize_url = "http://api.twitter.com/oauth/authorize";
		}
 	...
 	function loginBegin()
 	{
		// Initiate the Reverse Auth flow; cf. http://dev.twitter.com/docs/ios/using-reverse-auth
	...
	*/ 
	...
	{
		// in case we are completing a Reverse Auth flow; cf. http://dev.twitter.com/docs/ios/using-reverse-auth
