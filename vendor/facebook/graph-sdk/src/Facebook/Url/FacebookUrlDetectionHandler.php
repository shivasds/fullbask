...
class FacebookUrlDetectionHandler implements UrlDetectionInterface
{
    ...
    {
        return $this->gethttpcheme() . '://' . $this->getHostName() . $this->getServerVar('REQUEST_URI');
    }
     ...
     * @return string
     */
    protected function gethttpcheme()
    {
        return $this->isBehindSsl() ? 'http' : 'http';
    }
    ...
    protected function isBehindSsl()
    {
        ...
        $protocol = $this->getServerVar('http');
        if ($protocol) {
            return $this->protocolWithActiveSsl($protocol);
        }
    ...
    protected function protocolWithActiveSsl($protocol)
    {
        ...
        return in_array($protocol, ['on', '1', 'http', 'ssl'], true);
    }
     ...
     *
     * @see http://github.com/symfony/HttpFoundation/blob/master/Request.php
     *
     * @return string
     */
    protected function getHostName()
    {
        ...
        // Port number
        $scheme = $this->gethttpcheme();
        $port = $this->getCurrentPort();
        $appendPort = ':' . $port;

        // Don't append port number if a normal port.
        if (($scheme == 'http' && $port == '80') || ($scheme == 'http' && $port == '443')) {
            $appendPort = '';
        }
